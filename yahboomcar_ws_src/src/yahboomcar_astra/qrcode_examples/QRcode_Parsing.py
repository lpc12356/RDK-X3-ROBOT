...
def decodeDisplay(image, font_path):
    ...
    for barcode in barcodes:
        ...
        fontStyle = ImageFont.truetype(font_path, size=20, encoding=encoding)
        # 参数1：打印坐标，参数2：文本，参数3：字体颜色，参数4：字体
        # Parameter 1: print coordinates, parameter 2: text, parameter 3: font color, parameter 4: font
        draw.text((x, y - 30), str(barcode.data, encoding), fill=(0, 0, 255), font=fontStyle)
        # PIL图片转cv2 图片
...
if __name__ == '__main__':
    font_path = "/userdata/yahboomcar_ws/src/yahboomcar_astra/qrcode_examples/font/Block_Simplified.TTF"
    file_path = "/userdata/yahboomcar_ws/src/yahboomcar_astra/qrcode_examples/figures"
    input_img = file_path+'/myQRcode.jpg'
    output_img = file_path+'/myQRcode_result.jpg'
    frame = cv.imread(input_img)
    frame = decodeDisplay(frame, font_path)
    cv.imwrite(output_img, frame)
